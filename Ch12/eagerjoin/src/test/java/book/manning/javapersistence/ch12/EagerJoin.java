...
import book.manning.javapersistence.ch12.eagerjoin.Bid;
import book.manning.javapersistence.ch12.eagerjoin.Item;
import book.manning.javapersistence.ch12.eagerjoin.User;
...
class EagerJoin {
    ...
    void fetchEagerJoin() {
        ...
        try (EntityManager em = emf.createEntityManager()) {
            ...
            assertThat(item.getBids()).hasSize(3);
            assertThat(item.getBids().iterator().next().getAmount()).isNotNull();

            assertThat(item.getSeller().getUsername()).isEqualTo("johndoe");
